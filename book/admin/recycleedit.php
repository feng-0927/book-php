...
if(!$book)
{
  show('该书籍记录不存在，请重新选择','recyclelist.php');
  exit;
}
...
if($_POST)
{
  ...
  $bookcheck = $db->select()->from("book")->where("title = '$title' AND id != $bookid")->find();

  //当书籍存在的时候
  if($bookcheck)
  {
    show("该书籍已经存在了，请重新修改","recycleedit.php?bookid=$bookid");
    exit;
  }
...
<html lang="en">
  ...
  <body> 
    ...
    <div class="content">
        <div class="header">
            <h1 class="page-title">确认回收</h1>
        </div>
        <ul class="breadcrumb">
            <li><a href="index.php">首页</a> <span class="divider">/</span></li>
            <li class="active">回收</li>
        </ul>
        ...
        <div class="container-fluid">
            <div class="row-fluid">
                ...
                <div class="btn-toolbar">
                    <button class="btn btn-primary" onClick="location='recyclelist.php'"><i class="icon-list"></i> 返回书籍回收</button>
                  <div class="btn-group">
                  </div>
                </div>
                    ...
                    <div id="myTabContent" class="tab-content">
                      <div class="tab-pane active in" id="home">
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <label>书籍标签</label>
                            <select required name="flag" class="input-xlarge">
                              <option value="">请选择</option>
                              <?php foreach($booklist as $item){?>
                              <option <?php echo $book['id']==$item['id'] ? "selected":"";?> value="<?php echo $item['flag'];?>"><?php echo $item['flag'];?></option>
                              <?php }?>
                            </select>
                            <label>书籍标题</label>
                            <input type="text" class="input-xxlarge" name="title" required placeholder="请输入书籍标题" value="<?php echo $book['title'];?>" />
                            <label>书籍作者</label>
                            ...
                            <label></label>
                            <input class="btn btn-primary" type="submit" value="确认回收" />
                        </form>
                      </div>
                  </div>
                </div>
