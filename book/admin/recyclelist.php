...
if($_POST)
{
  ...
  if($affect)
  {
    ...
    show("删除书籍{$affect}条","recyclelist.php");
    exit;
  }else{
    show("删除书籍失败","recyclelist.php");
    exit;
  }
...
//总条数
$count = $db->select("COUNT(id) AS c")->from("book")->where("recycle = 'hide'")->find();
...
//查询数据
$booklist = $db->select("book.*,cate.name")->from("book")->join("cate","book.cateid = cate.id")->where("recycle = 'hide'")->orderby("book.id","desc")->limit($start,$limit)->all();
                      ...
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>
                                <a href="recycleedit.php?bookid=<?php echo $item['id'];?>"><i class="icon-pencil"></i></a>
